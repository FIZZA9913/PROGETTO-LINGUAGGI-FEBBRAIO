...
Per eseguire quest' operazione, però, la funzione jsonaccess si appoggia al predicato jsonaccess_execute. 
...
Il funzionamento di questo predicato non cambia dalla sua variante con jsonarray: il fine ultimo è quello di usare la variabile Field per ottenere il RisultatoFinale.
...
Le coppie sono strutture del tipo '(Attributo, Valore)', dove l' Attributo è una stringa SWI Prolog, mentre il Valore può essere a sua volta una stringa SWI Prolog,
oppure un numero, oppure true, false o null, oppure un oggetto json come jsonobject o jsonarray.
...
jsonaccess_execute che continua la ricerca, risalendo il contenuto della lista [Field | Fields] fino a quando quest'ultima non viene svuotata.
