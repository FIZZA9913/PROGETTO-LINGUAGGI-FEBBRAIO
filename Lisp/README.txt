...
Un utilizzo delle funzioni precedentemente citate con parametri diversi da quelli aspettati potrebbe causare 
...
creato da jsonparse.
Ogni funzione utilizzabile anche al di fuori delle funzioni richieste nel testo del progetto
controlla i suoi input, quindi parametri inaspettati non generano eccezioni incontrollate.
...
ed è poi passata alla funzione di supporto 'jsonparse-ex' che si occuperà del riconoscimento
...
;; jsonparse-ex
La funzione riceve in input una lista di codici di caratteri e gestisce una serie di casistiche diverse:
...
Se il parametro in input non è una stringa viene ritornato 'nil'.
...
;; conv-ls-str
Verifica che l'input sia una lista di codici di caratteri, produce una stringa invocando 'conv-ls-str-ex',
una funzione di appoggio a cui viene passata una stringa vuota da riempire.
 
;; conv-ls-str-ex
Accetta in input una lista di codici di caratteri e una stringa, produce una stringa dalla lista di caratteri ricorsivamente
con l'ausilio della macro code-char.
Se la lista di codici di caratteri è vuota la funzione ritorna "".

;; p-vl
La funzione riceve in input una lista di codici di caratteri e crea una nuova lista attraverso
...
;; p-obj
Accetta in input una lista di codici di caratteri e richiama la funzione ausiliaria p-obj-ex
passando gli argomenti necessari a preparare lo stato iniziale dell'automa a stati finiti.
...
Stati:
	...
	o1: Stato finale (con } riconosce un oggetto vuoto, altrimenti riconosce la chiave e va in o2).
	o2: Stato intermedio (con : riconosce il valore di un oggetto, lo appende alla lista p e va in o3).
	o3: Stato finale (con , riconosce il valore di un oggetto, la appende alla lista p e va in o2, con 
	} riconosce l'oggetto).
Se la lista di codici di caratteri in input è vuota o se nessuna delle condizioni è verificata viene ritornato errore.

;; p-arr
Accetta in input una lista di codici di caratteri, richiama la funzione ausiliaria p-arr-ex
...
Stati:
	...
	a1: Stato finale (riconosce value e passa in a2, con ] riconosce l'array vuoto).
...
;; p-str-ex
Accetta una stringa di lunghezza maggiore o uguale a 2 (lunghezza di "" è 2) in input,
ritorna la sotto-stringa contenuta tra i primi due apici doppi e la lista di codici rimanenti.
Se non è possibile riconoscere una stringa o la lunghezza è inferiore a 2 viene ritornato errore.
...
;; p-num-ex 
La funzione riceve in input una lista, una stringa e ritorna il numero json e
la lista di codici rimanenti.

;; p-true (p-false, p-null)
Accetta in input una lista di codici di caratteri e richiama la funzione aux p-true-ex.

;; p-true-ex (p-false-ex, p-null-ex)
La funzione riceve in input una stringa. Se all'interno della stringa è presente "true", "false" o "null"
estrae la sotto-stringa e la converte la restante parte in una lista di codici di caratteri.
Se all'interno della stringa non è presente uno dei valori booleani o la stringa non è lunga almeno 4 o 5 caratteri viene
ritornato un errore.
...
1) il primo elemento della lista è un jsonobject - viene invocata la funzione 'jsonobj'
2) il primo elemento della lista è un jsonarray - viene invocata la funzione 'jsonarray'
Se la lista in input è vuota o il primo elemento della lista non appartiene alle casistiche qui sopra
viene ritornato errore.

;; jsonobj
La funzione controlla se ciò che ha ricevuto in input è una lista di coppie; 
se tale condizione è verificata la lista viene passata alla funzione d'appoggio 'jsonobj-ex' assieme alla
stringa di inizio jsonobject "{", altrimenti viene ritornato un errore.

;; jsonobj-ex
La funzione riceve in input una lista di coppie e traduce il jsonobject in una stringa.
Produce ricorsivamente le coppie, inserisce la virgola dove richiesto con la funzione virgola, effettua la chiamata ricorsiva.
...
- Se il valore è un numero - un intero o float - viene scritto in notazione esponenziale.
...
Viene ritornato altrimenti un errore.
...
attraverso l'implementazione di apici doppi, a cui seguirà la virgola (se necessaria).
- Elemento = numero : la stringa 'trd' risultante conterrà l'elemento in notazione esponenziale e la virgola (se necessaria).
- Elemento = true/false/null : la stringa 'trd' risultante conterrà il valore booleano corrispondente 
seguito da una virgola (se necessaria).
