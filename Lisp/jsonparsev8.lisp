...
(defun jsonaccess (ls &rest fields)
  (cond ((and (stringp (trad-inv ls))
              (eql (first ls) 'jsonarray)
              (null fields))
         (error "Impossibile accedere ai dati"))
        ((and (stringp (trad-inv ls))
              (ver-fields fields))
         (jsonaccess-ex ls fields))
        (t
         (error "fields e' scritta in formato scorretto"))))

(defun jsonaccess-ex (ls fields)
  (cond ((null fields)
         ls)
        ;; object
        ((and (eql (first ls)
